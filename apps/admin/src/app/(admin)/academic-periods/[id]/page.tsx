    ...
    <div className="flex flex-col gap-6">
      ...
      <form action={updateAcademicPeriodAction.bind(null, period.id)} className="space-y-5 rounded-xl border border-slate-200 bg-white p-5">
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          ...
          <label className="space-y-1"><span className="text-sm font-medium text-slate-700">Data inicial</span><input name="startDate" type="date" required defaultValue={toDateInputValue(period.startDate)} disabled={!canWrite} className="h-9 w-full rounded-lg border border-slate-300 px-3 text-sm disabled:bg-slate-100" /></label>
          <label className="space-y-1"><span className="text-sm font-medium text-slate-700">Data final</span><input name="endDate" type="date" required defaultValue={toDateInputValue(period.endDate)} disabled={!canWrite} className="h-9 w-full rounded-lg border border-slate-300 px-3 text-sm disabled:bg-slate-100" /></label>
          <label className="space-y-1 sm:col-span-2">
